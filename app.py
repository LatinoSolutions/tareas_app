...
def save_posts(p): save_json(POSTS_FILE, p)

def load_lib(): return load_json(DATA_FILE, {})

def save_lib(l): save_json(DATA_FILE, l)
...
def open_detail(pid): st.session_state.update({"page":"Detalle","detail_id":pid})
...
def share_to_feed(fn, tn, t, posts):
    ...
    posts.append(post); save_posts(posts); st.success("Compartido ✔️")
